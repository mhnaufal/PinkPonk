...
func _on_Left_body_entered(body):
	...
	if PlayerScore==5 or OpponentScore==5:
		if PlayerScore>OpponentScore:
			get_tree().change_scene("res://scenes/Victory.tscn")	#Player
		else:
			get_tree().change_scene("res://scenes/Lose.tscn")	#Opponent
	$Timer.start()
	$Countdown.visible = true
	$Player.position.y = 360
	$opponents2.position.y = 360
	$Player.position.x = 72
	$opponents2.position.x = 1280 - 72
